...
const navItems = [
    ...
    { to: "/admin/catalogo", label: "Admin catálogo", roles: ["admin", "moderator"] },
    { to: "/admin/passagens-biblicas", label: "Passagens bíblicas", roles: ["admin", "moderator"] },
    { to: "/admin/utilizadores", label: "Admin utilizadores", roles: ["admin"] },
    { to: "/admin/metricas", label: "Métricas", roles: ["admin"] },
    { to: "/admin/integracoes", label: "Integrações", roles: ["admin"] },
];
 ...
 *
 * @param {{ visibility?: string, roles?: string[] }} item Item de navegação.
 * @param {{ status: string, hasRole: (allowedRoles: string[]) => boolean }} session Estado de sessão.
...
function canShowNavItem(item, session) {
    ...
    if (item.roles) return session.status === "authenticated" && session.hasRole(item.roles);
    return false;
}
