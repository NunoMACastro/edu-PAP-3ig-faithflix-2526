/**
 * @file Ficheiro `real_dev/frontend/src/pages/AdminPoolDistributionPage.jsx` da implementação real_dev.
 */
 ...
 * persistido sem recalcular valores críticos no navegador.
 */
...
/**
 * Formata valores monetários persistidos em cêntimos com convenções portuguesas.
 *
    ...
    <section className="page-section">
      ...
      {distribution && (
        <section>
          ...
          <p>Total: {formatMoney(distribution.totalPoolCents)}</p>
          {distribution.items.map((item) => (
            <article key={item.charityId}>
              <h3>{item.charityName}</h3>
              <p>{formatMoney(item.amountCents)}</p>
            </article>
          ))}
        </section>
      )}
