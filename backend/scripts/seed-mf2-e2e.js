...
if (existingUser) {
    ...
    await db
        ...
        .deleteMany({ userId: existingUser._id });
    await db.collection("subscriptions").deleteMany({ userId: existingUser._id });
    await db.collection("trials").deleteMany({ userId: existingUser._id });
...
await db.collection("subscriptions").deleteMany({ e2eFixture: E2E_TAG });
await db.collection("trials").deleteMany({ e2eFixture: E2E_TAG });
