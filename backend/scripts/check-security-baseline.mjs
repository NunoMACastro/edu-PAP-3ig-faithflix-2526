...
/**
 * Verifica se uma linha contém uma chamada direta a `eval`.
 *
 * Esta regra existe porque `eval` executa texto como código JavaScript e abre
 * caminho a execução dinâmica insegura quando algum valor chega de input externo.
 *
 * @param {string} line Linha de código fonte analisada pelo scanner.
 * @returns {boolean} `true` quando a linha contém uma chamada `eval(...)`.
 */
function hasEvalCall(line) {
    return /\beval\s*\(/u.test(line);
}

/**
 * Verifica se uma linha tenta apagar todos os documentos de uma coleção MongoDB.
 *
 * Um `deleteMany({})` sem filtro é perigoso em código aplicacional porque pode
 * remover dados de produção por acidente ou por abuso de uma rota vulnerável.
 *
 * @param {string} line Linha de código fonte analisada pelo scanner.
 * @returns {boolean} `true` quando a linha contém `deleteMany({})`.
 */
function hasUnfilteredDeleteMany(line) {
    return /deleteMany\s*\(\s*\{\s*\}\s*\)/u.test(line);
}
...
async function listSourceFiles(directory) {
    ...
    const files = await Promise.all(
        entries.map(async (entry) => {
            ...
            // O scanner protege o bundle/runtime. Testes colocados junto do
            // source contêm deliberadamente tokens e URIs sintéticas para
            // provar redaction e não são configuração executável do produto.
            if (
                /\.(js|jsx|mjs)$/u.test(entry.name) &&
                !/\.(?:test|spec)\.(?:js|jsx|mjs)$/u.test(entry.name)
            ) {
                return [absolutePath];
            }
