...
function createCollection(rows) {
  return {
    ...
    /**
     * Aplica o subset de `findOneAndUpdate` usado pelo rate limiter distribuído.
     *
     * @param {Record<string, unknown>} query Chave do contador.
     * @param {Record<string, Record<string, unknown>>} update Operadores MongoDB.
     * @returns {Promise<Record<string, unknown>>} Documento depois do update.
     */
...
function createTestDb() {
  ...
  const userId = new ObjectId("64f400000000000000000002");
  collections = {
    ...
    pool_distributions: createCollection([
      {
        _id: new ObjectId("64f400000000000000000003"),
        month: "2026-01",
        status: "completed",
        totalPoolCents: 2400,
      },
      {
        _id: new ObjectId("64f400000000000000000004"),
        month: "2026-02",
    ...
    sessions: createCollection([
      {
        _id: new ObjectId("64f400000000000000000005"),
        userId,
    ...
    users: createCollection([
      {
        _id: userId,
        name: "Representante",
        email: "representante@example.test",
        role: "user",
...
test("GET /api/charities/public devolve apenas associações públicas", async () => {
  ...
  assert.deepEqual(body.impact, {
    eligibleCharities: 1,
    totalDistributedCents: 2400,
    completedMonths: 1,
    currency: "EUR",
  });
  assert.equal(body.impact.paymentSnapshots, undefined);
});

test("GET /api/charities/me devolve apenas a associação da sessão", async () => {
  const response = await fetch(`${testServer.baseUrl}/api/charities/me`, {
    headers: {
      cookie: `${sessionConfig.cookieName}=${sessionToken}`,
    },
  });
  const body = await response.json();
...
test("rotas privadas da MF4 exigem autenticação", async () => {
  const [applicationsResponse, dashboardResponse, mineResponse, historyResponse] =
    await Promise.all([
      fetch(`${testServer.baseUrl}/api/charities/applications`),
      fetch(`${testServer.baseUrl}/api/charities/pool/dashboard`),
      fetch(`${testServer.baseUrl}/api/charities/me`),
      fetch(
  ...
  assert.equal(mineResponse.status, 401);
  assert.equal(historyResponse.status, 401);
});

test("POST /api/subscriptions/me permanece indisponivel sem checkout simulado", async () => {
  const response = await fetch(`${testServer.baseUrl}/api/subscriptions/me`, {
    method: "POST",
    headers: { "content-type": "application/json" },
