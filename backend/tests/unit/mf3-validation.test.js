...
test("ratings validam escala canonica de 1 a 5", () => {
    ...
    assert.throws(() => assertRatingValue("3"), /rating/);
...
test("pesquisa usa filtro published e desempate _id em todas as ordenacoes", async () => {
    ...
    for (const sort of ["title", "recent", "rating"]) {
        ...
        const sortStage = observedPipeline.find((stage) => stage.$sort)?.$sort;

        assert.equal(response.total, 0);
        assert.equal(match.status, "published");
        assert.equal(sortStage._id, 1);
    }

    setDbForTests(null);
});

test("pesquisa ignora taxonomyIds legacy invalidos ao resolver nomes", async () => {
    const validTaxonomyId = new ObjectId();
    const contentId = new ObjectId();
    let loadedTaxonomyIds;
            ...
            find(query) {
                return {
                    async toArray() {
                        ...
                        loadedTaxonomyIds = query._id.$in;
                        return [
                            { _id: validTaxonomyId, name: "Esperanca" },
                        ];
                    },
                };
            },
                                ...
                                items: [
                                    {
                                        ...
                                        title: "Esperanca",
                                        slug: "esperanca",
                                        synopsis: "Conteudo publicado de teste.",
                                        type: "movie",
                                        taxonomyIds: [
                                            validTaxonomyId,
                                            "legacy-invalido",
    ...
    const response = await searchContents({ q: "esperanca" });

    assert.deepEqual(response.items[0].taxonomyNames, ["Esperanca"]);
    assert.equal(loadedTaxonomyIds.length, 1);
    assert.equal(String(loadedTaxonomyIds[0]), String(validTaxonomyId));
    setDbForTests(null);
});
