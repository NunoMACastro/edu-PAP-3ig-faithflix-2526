 ...
 * @returns {Promise<{ deleted: true, removed: Record<string, number>, commentsAnonymized: number, subscriptionsCanceled: number }>} Resultado operacional.
 */
export async function deleteMyAccount(userId, input) {
    ...
    const now = new Date();
    const [removed, commentsAnonymized, subscriptionsCanceled] =
        await Promise.all([
