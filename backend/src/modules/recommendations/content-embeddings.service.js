 ...
 *
 * @param {{ force?: boolean, generatedAt?: Date, documentIdFactory?: (content: object) => ObjectId|string }} [options={}] Opções de geração.
...
export async function generateContentEmbeddings(options = {}) {
    ...
    for (const content of contents) {
        ...
        if (options.generatedAt instanceof Date) document.generatedAt = options.generatedAt;
        ...
        const documentId = options.documentIdFactory?.(content);
        await db.collection("content_embeddings").updateOne(
            { contentId: content._id, model: CONTENT_EMBEDDING_MODEL },
            {
                $set: document,
                ...(documentId ? { $setOnInsert: { _id: new ObjectId(String(documentId)) } } : {}),
            },
            { upsert: true },
        );
        summary.generated += 1;
