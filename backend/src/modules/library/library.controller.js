...
export async function getFavorites(req, res) {
    return res
        .status(200)
        .json(await listSavedContent(req.user.id, "favorite", req.query));
}
...
export async function getWatchlist(req, res) {
    return res
        .status(200)
        .json(await listSavedContent(req.user.id, "watchlist", req.query));
}
...
export async function getHistory(req, res) {
    return res.status(200).json(await listHistory(req.user.id, req.query));
}
