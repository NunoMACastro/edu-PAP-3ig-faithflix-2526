...
catalogRouter.get(
    "/admin/options",
    canManageCatalog,
    asyncHandler(getAdminCatalogEditorOptions),
);
catalogRouter.get(
    "/admin/:id",
    canManageCatalog,
    asyncHandler(getAdminCatalogDetail),
);
catalogRouter.get("/taxonomies", asyncHandler(getTaxonomies));
catalogRouter.get(
    "/taxonomies/admin",
    canManageCatalog,
    asyncHandler(getAdminTaxonomies),
);
catalogRouter.post("/taxonomies", canManageCatalog, asyncHandler(postTaxonomy));
catalogRouter.patch(
    "/taxonomies/:taxonomyId",
    canManageCatalog,
    asyncHandler(patchTaxonomy),
);
catalogRouter.patch(
    "/taxonomies/:taxonomyId/status",
    canManageCatalog,
    asyncHandler(patchTaxonomyStatus),
);
