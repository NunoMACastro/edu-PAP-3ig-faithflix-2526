...
- Consolidar a entrega de `Pesquisa unificada` com rastreabilidade explicita para `RF22`.
- Executar o BK `BK-MF3-03` no contexto da macro `MF3` e da sprint `S05`.
...
Entregar `Pesquisa unificada` cobrindo `RF22` na `MF3`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF3-04`).
...
- Entrega funcional de `Pesquisa unificada` com caminho principal completo.
...
- Fluxo principal de `BK-MF3-03` reproduzivel por outro colega.
...
- `docs/RF.md` e `docs/RNF.md` (itens de `RF22`).
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF3-03`).
...
3. Implementar o fluxo principal de `Pesquisa unificada`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P0`.
6. Atualizar evidence e preparar handoff para `BK-MF3-04`.

## Outputs esperados

- Output funcional de `BK-MF3-03` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF3-03
precondicoes_ok = validar_dependencias(["BK-MF2-03"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Pesquisa unificada")
...
- Condicao: fluxo principal de `BK-MF3-03` concluido ponta-a-ponta.
