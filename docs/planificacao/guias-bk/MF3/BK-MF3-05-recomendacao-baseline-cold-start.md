...
- Consolidar a entrega de `Recomendacao baseline + cold start` com rastreabilidade explicita para `RF26, RF27`.
- Executar o BK `BK-MF3-05` no contexto da macro `MF3` e da sprint `S06`.
...
Entregar `Recomendacao baseline + cold start` cobrindo `RF26, RF27` na `MF3`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF3-06`).
...
- Dependencias declaradas: `BK-MF3-01,BK-MF2-07`.
...
- Entrega funcional de `Recomendacao baseline + cold start` com caminho principal completo.
...
- Fluxo principal de `BK-MF3-05` reproduzivel por outro colega.
...
- `docs/RF.md` e `docs/RNF.md` (itens de `RF26, RF27`).
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF3-05`).
...
3. Implementar o fluxo principal de `Recomendacao baseline + cold start`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P1`.
6. Atualizar evidence e preparar handoff para `BK-MF3-06`.

## Outputs esperados

- Output funcional de `BK-MF3-05` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF3-05
precondicoes_ok = validar_dependencias(["BK-MF3-01","BK-MF2-07"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Recomendacao baseline + cold start")
...
- Condicao: fluxo principal de `BK-MF3-05` concluido ponta-a-ponta.
