# BK-MF3-05 - Recomendação baseline + cold start
...
- estado: TODO
- esforço: L
- dependências: BK-MF3-01,BK-MF2-07
- rf_rnf: RF26,RF27
- last_updated: 2026-04-12

## O que vamos fazer neste BK

Neste BK vamos entregar Recomendação baseline + cold start, com foco num fluxo claro e executável para alunos em contexto de PAP.
...
- Este BK fecha uma peça concreta da macro MF3 e reduz risco de bloqueios nos BKs seguintes.
- Uma execução guiada melhora consistência entre equipa, reduz retrabalho e acelera revisão técnica.
- A entrega deste BK contribui diretamente para os requisitos RF26,RF27 definidos no backlog.
- Sem este BK bem fechado, a qualidade das evidências para sprint e defesa final fica comprometida.

## O que entra (scope)

- Entregar o objetivo central: Recomendação baseline + cold start.
- Produzir artefacto verificável alinhado com RF26,RF27 e com o estado atual do backlog.
...
- As dependências BK-MF3-01,BK-MF2-07 foram respeitadas sem regressão funcional óbvia.
- A equipa consegue explicar o porquê da solução e não apenas o como.

## Pre-leitura mínima (10-15 min)

- docs/planificacao/backlogs/BACKLOG-MVP.md - localizar a linha do BK-MF3-05, owner, esforço e dependências.
- docs/planificacao/PLANO-IMPLEMENTACAO-TOTAL.md - rever secção da macro MF3.
- docs/planificacao/backlogs/MF-VIEWS.md - confirmar sequência recomendada na macro.
- docs/RF.md e/ou docs/RNF.md - focar os códigos RF26,RF27.
...
1. **Objetivo (~10 min): confirmar ponto de partida**
   ...
   - Como fazer: validar se BK-MF3-01,BK-MF2-07 está concluído ou suficientemente estável para suportar este BK.
   - O que verificar: existe baseline claro para iniciar BK-MF3-05.
...
2. **Objetivo (~15 min): traduzir o requisito em tarefas pequenas**
   ...
   - Como fazer: decompor Recomendação baseline + cold start em 3 a 5 tarefas objetivas (entrada, processamento, saída, validação, evidência).
...
3. **Objetivo (~25 min): desenhar fluxo e critérios antes de implementar**
   ...
   - O que verificar: fluxo alinhado com RF26,RF27 e sem invadir BKs adjacentes.
...
- **Smoke**
  - O objetivo Recomendação baseline + cold start pode ser demonstrado de ponta a ponta no contexto atual.
  - Não há bloqueio imediato para o BK-MF3-06.
- **Negativos**
  - Existe pelo menos 1 teste de erro/validação diretamente ligado a Recomendação baseline + cold start.
  - Falhas esperadas ficam explícitas e sem comportamento ambíguo.
- **Técnico**
  - Alinhamento confirmado com RF26,RF27.
...
- proof: BK BK-MF3-05 documentado com guia pedagógico completo, alinhado com backlog e requisitos associados.
