# BK-MF3-07 - Estudo biblico e guias
...
- `guia_path`: `docs/planificacao/guias-bk/MF3/BK-MF3-07-estudo-biblico-e-guias.md`
...
- Consolidar a entrega de `Estudo biblico e guias` com rastreabilidade explicita para `RF29, RF30, RF31`.
- Executar o BK `BK-MF3-07` no contexto da macro `MF3` e da sprint `S06`.
...
Entregar `Estudo biblico e guias` cobrindo `RF29, RF30, RF31` na `MF3`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF3-08`).
...
- Entrega funcional de `Estudo biblico e guias` com caminho principal completo.
...
- Fluxo principal de `BK-MF3-07` reproduzivel por outro colega.
...
- `docs/RF.md` e `docs/RNF.md` (itens de `RF29, RF30, RF31`).
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF3-07`).
...
3. Implementar o fluxo principal de `Estudo biblico e guias`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P1`.
6. Atualizar evidence e preparar handoff para `BK-MF3-08`.

## Outputs esperados

- Output funcional de `BK-MF3-07` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF3-07
precondicoes_ok = validar_dependencias(["BK-MF2-04"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Estudo biblico e guias")
...
- Condicao: fluxo principal de `BK-MF3-07` concluido ponta-a-ponta.
