...
- Consolidar a entrega de `Ratings e agregacao` com rastreabilidade explicita para `RF19, RF21`.
- Executar o BK `BK-MF3-01` no contexto da macro `MF3` e da sprint `S05`.
...
Entregar `Ratings e agregacao` cobrindo `RF19, RF21` na `MF3`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF3-02`).
...
- Dependencias declaradas: `BK-MF2-07`.
...
- Entrega funcional de `Ratings e agregacao` com caminho principal completo.
...
- Fluxo principal de `BK-MF3-01` reproduzivel por outro colega.
...
- `docs/RF.md` e `docs/RNF.md` (itens de `RF19, RF21`).
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF3-01`).
...
3. Implementar o fluxo principal de `Ratings e agregacao`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P1`.
6. Atualizar evidence e preparar handoff para `BK-MF3-02`.

## Outputs esperados

- Output funcional de `BK-MF3-01` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF3-01
precondicoes_ok = validar_dependencias(["BK-MF2-07"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Ratings e agregacao")
...
- Condicao: fluxo principal de `BK-MF3-01` concluido ponta-a-ponta.
