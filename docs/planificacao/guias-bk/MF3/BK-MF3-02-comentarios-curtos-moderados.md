...
Entregar `Comentarios curtos moderados` cobrindo `RF20` na `MF3`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF3-03`).
...
- Entrega funcional de `Comentarios curtos moderados` com caminho principal completo.
...
- Fluxo principal de `BK-MF3-02` reproduzivel por outro colega.
...
3. Implementar o fluxo principal de `Comentarios curtos moderados`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P2`.
6. Atualizar evidence e preparar handoff para `BK-MF3-03`.

## Outputs esperados

- Output funcional de `BK-MF3-02` concluido sem blocker.
...
precondicoes_ok = validar_dependencias(["BK-MF3-01"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Comentarios curtos moderados")
...
- Condicao: fluxo principal de `BK-MF3-02` concluido ponta-a-ponta.
