...
Entregar `Filtros, carrosseis e relacionados` cobrindo `RF23, RF24, RF25` na `MF3`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF3-05`).
...
- Entrega funcional de `Filtros, carrosseis e relacionados` com caminho principal completo.
...
- Fluxo principal de `BK-MF3-04` reproduzivel por outro colega.
...
3. Implementar o fluxo principal de `Filtros, carrosseis e relacionados`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P1`.
6. Atualizar evidence e preparar handoff para `BK-MF3-05`.

## Outputs esperados

- Output funcional de `BK-MF3-04` concluido sem blocker.
...
precondicoes_ok = validar_dependencias(["BK-MF3-03"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Filtros, carrosseis e relacionados")
...
- Condicao: fluxo principal de `BK-MF3-04` concluido ponta-a-ponta.
