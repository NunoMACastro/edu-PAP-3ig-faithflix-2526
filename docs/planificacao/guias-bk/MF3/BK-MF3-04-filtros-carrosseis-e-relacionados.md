...
Neste BK vamos entregar Filtros, carrosseis e relacionados, com foco num fluxo claro e executável para alunos em contexto de PAP.
...
- A entrega deste BK contribui diretamente para os requisitos RF23,RF24,RF25 definidos no backlog.
- Sem este BK bem fechado, a qualidade das evidências para sprint e defesa final fica comprometida.

## O que entra (scope)

- Entregar o objetivo central: Filtros, carrosseis e relacionados.
- Produzir artefacto verificável alinhado com RF23,RF24,RF25 e com o estado atual do backlog.
...
- As dependências BK-MF3-03 foram respeitadas sem regressão funcional óbvia.
- A equipa consegue explicar o porquê da solução e não apenas o como.

## Pre-leitura mínima (10-15 min)

- docs/planificacao/backlogs/BACKLOG-MVP.md - localizar a linha do BK-MF3-04, owner, esforço e dependências.
...
- docs/RF.md e/ou docs/RNF.md - focar os códigos RF23,RF24,RF25.
...
1. **Objetivo (~10 min): confirmar ponto de partida**
   ...
   - Como fazer: validar se BK-MF3-03 está concluído ou suficientemente estável para suportar este BK.
   - O que verificar: existe baseline claro para iniciar BK-MF3-04.
...
2. **Objetivo (~15 min): traduzir o requisito em tarefas pequenas**
   ...
   - Como fazer: decompor Filtros, carrosseis e relacionados em 3 a 5 tarefas objetivas (entrada, processamento, saída, validação, evidência).
...
3. **Objetivo (~25 min): desenhar fluxo e critérios antes de implementar**
   ...
   - O que verificar: fluxo alinhado com RF23,RF24,RF25 e sem invadir BKs adjacentes.
...
- **Smoke**
  - O objetivo Filtros, carrosseis e relacionados pode ser demonstrado de ponta a ponta no contexto atual.
  - Não há bloqueio imediato para o BK-MF3-05.
- **Negativos**
  - Existe pelo menos 1 teste de erro/validação diretamente ligado a Filtros, carrosseis e relacionados.
  - Falhas esperadas ficam explícitas e sem comportamento ambíguo.
- **Técnico**
  - Alinhamento confirmado com RF23,RF24,RF25.
...
- proof: BK BK-MF3-04 documentado com guia pedagógico completo, alinhado com backlog e requisitos associados.
