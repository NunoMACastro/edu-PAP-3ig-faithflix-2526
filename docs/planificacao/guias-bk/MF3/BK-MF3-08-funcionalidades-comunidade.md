...
- `guia_path`: `docs/planificacao/guias-bk/MF3/BK-MF3-08-funcionalidades-comunidade.md`
...
- Consolidar a entrega de `Funcionalidades comunidade` com rastreabilidade explicita para `RF32, RF33, RF34`.
- Executar o BK `BK-MF3-08` no contexto da macro `MF3` e da sprint `S06`.
...
Entregar `Funcionalidades comunidade` cobrindo `RF32, RF33, RF34` na `MF3`, com fluxo principal verificavel e evidencia tecnica pronta para gate.
...
- Entrega funcional de `Funcionalidades comunidade` com caminho principal completo.
...
- Fluxo principal de `BK-MF3-08` reproduzivel por outro colega.
...
- `docs/RF.md` e `docs/RNF.md` (itens de `RF32, RF33, RF34`).
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF3-08`).
...
3. Implementar o fluxo principal de `Funcionalidades comunidade`.
...
- Output funcional de `BK-MF3-08` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF3-08
precondicoes_ok = validar_dependencias(["BK-MF3-02"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Funcionalidades comunidade")
...
- Condicao: fluxo principal de `BK-MF3-08` concluido ponta-a-ponta.
