...
- `guia_path`: `docs/planificacao/guias-bk/MF8/BK-MF8-03-scope-freeze-final.md`
...
- Consolidar a entrega de `Scope freeze final` com rastreabilidade explicita para `transversal`.
- Executar o BK `BK-MF8-03` no contexto da macro `MF8` e da sprint `S12`.
...
Entregar `Scope freeze final` cobrindo `transversal` na `MF8`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF8-04`).
...
- Dependencias declaradas: `BK-MF8-02`.
...
- Entrega funcional de `Scope freeze final` com caminho principal completo.
...
- Fluxo principal de `BK-MF8-03` reproduzivel por outro colega.
...
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF8-03`).
...
3. Implementar o fluxo principal de `Scope freeze final`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P0`.
6. Atualizar evidence e preparar handoff para `BK-MF8-04`.

## Outputs esperados

- Output funcional de `BK-MF8-03` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF8-03
precondicoes_ok = validar_dependencias(["BK-MF8-02"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Scope freeze final")
...
- Condicao: fluxo principal de `BK-MF8-03` concluido ponta-a-ponta.
