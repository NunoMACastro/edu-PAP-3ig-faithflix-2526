# BK-MF8-01 - Lista de riscos residuais
...
- `guia_path`: `docs/planificacao/guias-bk/MF8/BK-MF8-01-lista-de-riscos-residuais.md`
...
- Consolidar a entrega de `Lista de riscos residuais` com rastreabilidade explicita para `transversal`.
- Executar o BK `BK-MF8-01` no contexto da macro `MF8` e da sprint `S12`.
...
Entregar `Lista de riscos residuais` cobrindo `transversal` na `MF8`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF8-02`).
...
- Dependencias declaradas: `BK-MF7-05`.
...
- Entrega funcional de `Lista de riscos residuais` com caminho principal completo.
...
- Fluxo principal de `BK-MF8-01` reproduzivel por outro colega.
...
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF8-01`).
...
3. Implementar o fluxo principal de `Lista de riscos residuais`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P0`.
6. Atualizar evidence e preparar handoff para `BK-MF8-02`.

## Outputs esperados

- Output funcional de `BK-MF8-01` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF8-01
precondicoes_ok = validar_dependencias(["BK-MF7-05"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Lista de riscos residuais")
...
- Condicao: fluxo principal de `BK-MF8-01` concluido ponta-a-ponta.
