...
| `TST-MF8-BE-REG-HARDENING` | Regressão/hardening backend | `backend/` | `npm test`; `node scripts/check-security-baseline.mjs` | Primeira regra de regressão ou hardening que falhou. |
| `TST-MF8-BE-ADMIN-ATOMIC` | Atomicidade administrativa | raiz | `node --test backend/tests/unit/f3-admin-transactions.test.js` | Total, primeiro cenário falhado e confirmação de rollback/concorrência sem dados pessoais. |
| `TST-MF8-FE-BUILD` | Build frontend | raiz | `npm --prefix frontend run build` | Build `PASS` ou erro de compilação. |
| `TST-MF8-FE-SMOKE` | Smoke frontend | raiz | `npm --prefix frontend run smoke` | Estado do smoke frontend e fluxo que falhou. |
| `TST-MF8-FE-REG` | Regressão frontend | `frontend/` | `node scripts/check-frontend-regression.mjs` | `PASS` ou primeira regra de regressão que falhou. |
...
| `TST-MF8-VISUAL-RESP` | Visual/responsivo | raiz | Build `frontend` + Playwright formal | Browser, viewport, screenshot em `test-results/` e observação; publicação em `docs` é separada. |
...
Este passo prepara execução e evidence.
...
A tarefa é executar comandos definidos e registar evidência.

Como não há código neste passo, a explicação incide sobre leitura de output:
falha de assertion indica comportamento incorreto; comando ausente indica
lacuna de setup; erro de ambiente deve ser classificado como `BLOQUEADO`. No
teste admin, estado parcial após fault injection ou ausência de admin ativo é
falha funcional, não ressalva documental.
...
Teste manual é procedimento com evidence, não alteração de implementação.
...
2. Ficheiros envolvidos:
    - GERAR: imagens normais em `test-results/`
    - PUBLICAR: em `docs/evidence/MF8/screenshots/` apenas depois de revisão explícita
    - EDITAR: `docs/evidence/MF8/EXECUCAO-TESTES-REPORT-ERROS.md`
...
A tarefa é recolher evidence visual e classificar resultados.
...
Este BK reporta; a correção fica para `BK-MF8-09`.
...
A decisão final é documental e operacional.
...
- `TST-MF8-BE-ADMIN-ATOMIC` tem resultado próprio e não é dado como coberto
  apenas porque a suite genérica passou.
- A execução administrativa regista rollback, concorrência, revogação de
  sessões, correlação por `requestId` e proteção do último admin ativo.
...
##### Adendo de execução `TST-MF8-A11Y-PREVIEW`

Para este ID, o report deve guardar comando, cwd, data, exit code e resumo real:

- total de cenários e engine utilizada;
- rotas/viewports, Axe `serious`/`critical`, overflow, reflow e teclado;
- budgets observados e confirmação de adapters media fora do chunk inicial;
- primeiro finding Axe real, correção e nova execução, quando existir;
- zero pedidos externos e confirmação de que não arrancou backend, DB ou seed;
- limitação explícita: não é full E2E, não prova streaming real e não fecha a
  matriz browser completa.

Um `PASS` deste ID só vale para as rotas e regras efetivamente executadas. Não
se reutilizam screenshots ou totais históricos como resultado atual.
...
- `2026-07-10`: paths públicos restaurados e execução administrativa
  transacional adicionada como ID obrigatório com prova independente.
- `2026-07-10`: execução final passou a exigir evidence autónoma do teste Axe
  preview-only e dos respetivos limites.
