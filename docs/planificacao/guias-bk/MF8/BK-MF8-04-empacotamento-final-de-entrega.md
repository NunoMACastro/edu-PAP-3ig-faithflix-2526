...
- `guia_path`: `docs/planificacao/guias-bk/MF8/BK-MF8-04-empacotamento-final-de-entrega.md`
...
- Consolidar a entrega de `Empacotamento final de entrega` com rastreabilidade explicita para `transversal`.
- Executar o BK `BK-MF8-04` no contexto da macro `MF8` e da sprint `S12`.
...
Entregar `Empacotamento final de entrega` cobrindo `transversal` na `MF8`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF8-05`).
...
- Dependencias declaradas: `BK-MF8-03`.
...
- Entrega funcional de `Empacotamento final de entrega` com caminho principal completo.
...
- Fluxo principal de `BK-MF8-04` reproduzivel por outro colega.
...
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF8-04`).
...
3. Implementar o fluxo principal de `Empacotamento final de entrega`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P1`.
6. Atualizar evidence e preparar handoff para `BK-MF8-05`.

## Outputs esperados

- Output funcional de `BK-MF8-04` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF8-04
precondicoes_ok = validar_dependencias(["BK-MF8-03"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Empacotamento final de entrega")
...
- Condicao: fluxo principal de `BK-MF8-04` concluido ponta-a-ponta.
