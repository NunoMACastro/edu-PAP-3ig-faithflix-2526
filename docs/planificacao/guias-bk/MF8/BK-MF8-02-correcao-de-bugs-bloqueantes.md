# BK-MF8-02 - Correcao de bugs bloqueantes
...
- `guia_path`: `docs/planificacao/guias-bk/MF8/BK-MF8-02-correcao-de-bugs-bloqueantes.md`
...
- Consolidar a entrega de `Correcao de bugs bloqueantes` com rastreabilidade explicita para `transversal`.
- Executar o BK `BK-MF8-02` no contexto da macro `MF8` e da sprint `S12`.
...
Entregar `Correcao de bugs bloqueantes` cobrindo `transversal` na `MF8`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF8-03`).
...
- Dependencias declaradas: `BK-MF8-01`.
...
- Entrega funcional de `Correcao de bugs bloqueantes` com caminho principal completo.
...
- Fluxo principal de `BK-MF8-02` reproduzivel por outro colega.
...
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF8-02`).
...
3. Implementar o fluxo principal de `Correcao de bugs bloqueantes`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P0`.
6. Atualizar evidence e preparar handoff para `BK-MF8-03`.

## Outputs esperados

- Output funcional de `BK-MF8-02` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF8-02
precondicoes_ok = validar_dependencias(["BK-MF8-01"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Correcao de bugs bloqueantes")
...
- Condicao: fluxo principal de `BK-MF8-02` concluido ponta-a-ponta.
