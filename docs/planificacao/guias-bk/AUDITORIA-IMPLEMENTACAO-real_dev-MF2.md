...
- `proof_scope`: adendos locais atuais e snapshot MF2 delimitado; E2E/DB reais continuam bloqueados

## Adendo pos-correcao do catalogo - 2026-07-10

Este adendo regista apenas o estado atual do catálogo depois do hardening da
Fase 3. O relatório abaixo permanece como snapshot histórico da auditoria MF2 e
os seus resultados/comandos antigos não são reutilizados como prova atual.

- Implementação docente verificada: `real_dev/backend/src/modules/catalog/`.
- Conteúdo novo inicia em `version: 1`; documentos legados sem versão são
  tratados como versão `1` apenas na primeira escrita CAS.
- As respostas administrativas incluem `version`. Edição, mudança de estado e
  reversão exigem `expectedVersion` numérico; strings e omissão devolvem `400`
  com `EXPECTED_VERSION_REQUIRED`.
- Uma versão obsoleta devolve `409 CONTENT_VERSION_CONFLICT`. Revisão, escrita
  CAS e audit log usam a mesma sessão de `runInTransaction`, pelo que uma falha
  não deixa revisão órfã nem conteúdo parcialmente atualizado.
- Publicar novamente um conteúdo já `published`, com a versão atual, é um no-op:
  não cria revisão, não incrementa `version` e preserva `publishedAt`.
- Conteúdo `published` pode continuar com `mediaStatus: "pending"`; catálogo e
  detalhe públicos devolvem `isPlayable: false` e zero fontes reproduzíveis.
- Prova atual focada: `node --check` dos módulos de catálogo e
  `node --test tests/unit/catalog-transactions.test.js tests/unit/media-boundary.test.js`
  em `real_dev/backend`, com `14/14` testes passados, sem DB ou rede.
- Limite da prova: os testes usam doubles locais com rollback; a transação numa
  topologia MongoDB replica set dedicada continua dependente do checkpoint CP3.

## Snapshot histórico — auditoria MF2 anterior à correção de 2026-07-10
