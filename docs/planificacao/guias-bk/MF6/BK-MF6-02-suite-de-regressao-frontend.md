# BK-MF6-02 - Suite de regressao frontend
...
- Consolidar a entrega de `Suite de regressao frontend` com rastreabilidade explicita para `RNF29`.
- Executar o BK `BK-MF6-02` no contexto da macro `MF6` e da sprint `S10`.
...
Entregar `Suite de regressao frontend` cobrindo `RNF29` na `MF6`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF6-03`).
...
- Entrega funcional de `Suite de regressao frontend` com caminho principal completo.
...
- Fluxo principal de `BK-MF6-02` reproduzivel por outro colega.
...
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF6-02`).
...
3. Implementar o fluxo principal de `Suite de regressao frontend`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P0`.
6. Atualizar evidence e preparar handoff para `BK-MF6-03`.

## Outputs esperados

- Output funcional de `BK-MF6-02` concluido sem blocker.
...
- Condicao: fluxo principal de `BK-MF6-02` concluido ponta-a-ponta.
