# BK-MF6-01 - Suite de regressao backend
...
Entregar `Suite de regressao backend` cobrindo `RNF29` na `MF6`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF6-02`).
...
- Entrega funcional de `Suite de regressao backend` com caminho principal completo.
...
- Fluxo principal de `BK-MF6-01` reproduzivel por outro colega.
...
3. Implementar o fluxo principal de `Suite de regressao backend`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P0`.
6. Atualizar evidence e preparar handoff para `BK-MF6-02`.

## Outputs esperados

- Output funcional de `BK-MF6-01` concluido sem blocker.
...
resultado = executar_fluxo_principal("Suite de regressao backend")
...
- Condicao: fluxo principal de `BK-MF6-01` concluido ponta-a-ponta.
