...
- Consolidar a entrega de `Publicar backlog atomico inicial` com rastreabilidade explicita para `transversal`.
- Executar o BK `BK-MF0-03` no contexto da macro `MF0` e da sprint `S01`.
...
Entregar `Publicar backlog atomico inicial` cobrindo `transversal` na `MF0`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF0-04`).
...
- Entrega funcional de `Publicar backlog atomico inicial` com caminho principal completo.
...
- Fluxo principal de `BK-MF0-03` reproduzivel por outro colega.
...
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF0-03`).
...
3. Implementar o fluxo principal de `Publicar backlog atomico inicial`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P0`.
6. Atualizar evidence e preparar handoff para `BK-MF0-04`.

## Outputs esperados

- Output funcional de `BK-MF0-03` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF0-03
precondicoes_ok = validar_dependencias(["BK-MF0-01"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Publicar backlog atomico inicial")
...
- Condicao: fluxo principal de `BK-MF0-03` concluido ponta-a-ponta.
