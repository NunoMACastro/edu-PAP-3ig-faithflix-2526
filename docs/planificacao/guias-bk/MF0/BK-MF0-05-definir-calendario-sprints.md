...
- Consolidar a entrega de `Definir calendario de sprints` com rastreabilidade explicita para `transversal`.
- Executar o BK `BK-MF0-05` no contexto da macro `MF0` e da sprint `S01`.
...
Entregar `Definir calendario de sprints` cobrindo `transversal` na `MF0`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF0-06`).
...
- Entrega funcional de `Definir calendario de sprints` com caminho principal completo.
...
- Fluxo principal de `BK-MF0-05` reproduzivel por outro colega.
...
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF0-05`).
...
3. Implementar o fluxo principal de `Definir calendario de sprints`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P0`.
6. Atualizar evidence e preparar handoff para `BK-MF0-06`.

## Outputs esperados

- Output funcional de `BK-MF0-05` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF0-05
precondicoes_ok = validar_dependencias(["BK-MF0-03"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Definir calendario de sprints")
...
- Condicao: fluxo principal de `BK-MF0-05` concluido ponta-a-ponta.
