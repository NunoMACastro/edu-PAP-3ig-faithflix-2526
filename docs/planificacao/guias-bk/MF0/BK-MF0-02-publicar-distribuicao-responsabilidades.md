# BK-MF0-02 - Publicar distribuição de responsabilidades
...
Neste BK vamos formalizar ownership, apoio, regras de colaboração e papel de orientação para toda a equipa.
...
- Entregar o objetivo central: Publicar distribuição de responsabilidades.
...
- As dependências BK-MF0-01 foram respeitadas sem regressão funcional óbvia.
- A equipa consegue explicar o "porquê" da solução e não apenas o "como".

## Pre-leitura mínima (10-15 min)

- docs/planificacao/backlogs/BACKLOG-MVP.md - localizar a linha do BK-MF0-02, owner, esforço e dependências.
...
1. **Objetivo (~10 min): confirmar ponto de partida**
   ...
   - Como fazer: validar se BK-MF0-01 está concluído ou suficientemente estável para permitir progresso real.
   - O que verificar: existe um baseline claro para iniciar BK-MF0-02.
...
2. **Objetivo (~15 min): traduzir requisito para tarefas pequenas**
   ...
   - Como fazer: decompor Publicar distribuição de responsabilidades em 3 a 5 tarefas objetivas (entrada, processamento, saída, validação, evidência).
...
- **Smoke**
  - O objetivo Publicar distribuição de responsabilidades pode ser demonstrado de ponta a ponta no contexto atual.
  - Não há bloqueio imediato para o BK-MF0-03.
- **Negativos**
  - Existe pelo menos 1 teste de erro/validação diretamente ligado a Publicar distribuição de responsabilidades.
...
- proof: BK BK-MF0-02 documentado com guia pedagógico completo, alinhado com backlog e requisitos associados.
