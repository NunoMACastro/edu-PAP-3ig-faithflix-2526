...
- Consolidar a entrega de `Publicar distribuicao de responsabilidades` com rastreabilidade explicita para `transversal`.
- Executar o BK `BK-MF0-02` no contexto da macro `MF0` e da sprint `S01`.
...
Entregar `Publicar distribuicao de responsabilidades` cobrindo `transversal` na `MF0`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF0-03`).
...
- Dependencias declaradas: `BK-MF0-01`.
...
- Entrega funcional de `Publicar distribuicao de responsabilidades` com caminho principal completo.
...
- Fluxo principal de `BK-MF0-02` reproduzivel por outro colega.
...
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF0-02`).
...
3. Implementar o fluxo principal de `Publicar distribuicao de responsabilidades`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P0`.
6. Atualizar evidence e preparar handoff para `BK-MF0-03`.

## Outputs esperados

- Output funcional de `BK-MF0-02` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF0-02
precondicoes_ok = validar_dependencias(["BK-MF0-01"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Publicar distribuicao de responsabilidades")
...
- Condicao: fluxo principal de `BK-MF0-02` concluido ponta-a-ponta.
