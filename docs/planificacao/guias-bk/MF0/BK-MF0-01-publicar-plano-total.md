...
Neste BK vamos publicar e validar o plano macro total da PAP (MF0..MF8), garantindo uma leitura orientada para execução.
...
- Entregar o objetivo central: Publicar plano total.
...
- As dependências - foram respeitadas sem regressão funcional óbvia.
- A equipa consegue explicar o "porquê" da solução e não apenas o "como".

## Pre-leitura mínima (10-15 min)

- docs/planificacao/backlogs/BACKLOG-MVP.md - localizar a linha do BK-MF0-01, owner, esforço e dependências.
...
1. **Objetivo (~10 min): confirmar ponto de partida**
   ...
   - Como fazer: validar se - está concluído ou suficientemente estável para permitir progresso real.
   - O que verificar: existe um baseline claro para iniciar BK-MF0-01.
...
2. **Objetivo (~15 min): traduzir requisito para tarefas pequenas**
   ...
   - Como fazer: decompor Publicar plano total em 3 a 5 tarefas objetivas (entrada, processamento, saída, validação, evidência).
...
- **Smoke**
  - O objetivo Publicar plano total pode ser demonstrado de ponta a ponta no contexto atual.
  - Não há bloqueio imediato para o BK-MF0-02.
- **Negativos**
  - Existe pelo menos 1 teste de erro/validação diretamente ligado a Publicar plano total.
...
- proof: BK BK-MF0-01 documentado com guia pedagógico completo, alinhado com backlog e requisitos associados.
