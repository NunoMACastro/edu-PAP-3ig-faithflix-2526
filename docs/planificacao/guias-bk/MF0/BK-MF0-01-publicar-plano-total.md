...
- Consolidar a entrega de `Publicar plano total` com rastreabilidade explicita para `transversal`.
- Executar o BK `BK-MF0-01` no contexto da macro `MF0` e da sprint `S01`.
...
Entregar `Publicar plano total` cobrindo `transversal` na `MF0`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF0-02`).
...
- Sem dependencias bloqueantes para iniciar este BK.
...
- Entrega funcional de `Publicar plano total` com caminho principal completo.
...
- Fluxo principal de `BK-MF0-01` reproduzivel por outro colega.
...
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF0-01`).
...
3. Implementar o fluxo principal de `Publicar plano total`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P0`.
6. Atualizar evidence e preparar handoff para `BK-MF0-02`.

## Outputs esperados

- Output funcional de `BK-MF0-01` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF0-01
precondicoes_ok = validar_dependencias([])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Publicar plano total")
...
- Condicao: fluxo principal de `BK-MF0-01` concluido ponta-a-ponta.
