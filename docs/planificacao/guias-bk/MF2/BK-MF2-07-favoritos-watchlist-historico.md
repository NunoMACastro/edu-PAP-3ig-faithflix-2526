# BK-MF2-07 - Favoritos/watchlist/histórico
...
Neste BK vamos entregar gestão de favoritos, watchlist e histórico de consumo de conteúdo.
...
- A execução deste BK liga diretamente os requisitos RF16,RF17,RF18 ao plano operacional do backlog.
- Sem este BK bem fechado, a validação de sprint fica frágil e a evidência para defesa perde qualidade.

## O que entra (scope)

- Entregar o objetivo central: Favoritos/watchlist/histórico.
- Produzir artefacto verificável alinhado com RF16,RF17,RF18 e com o estado do backlog.
...
- docs/planificacao/backlogs/BACKLOG-MVP.md - localizar a linha do BK-MF2-07, owner, esforço e dependências.
...
- docs/RF.md e/ou docs/RNF.md - focar os códigos RF16,RF17,RF18.
...
1. **Objetivo (~10 min): confirmar ponto de partida**
   ...
   - Como fazer: validar se BK-MF2-05 está concluído ou suficientemente estável para permitir progresso real.
   - O que verificar: existe um baseline claro para iniciar BK-MF2-07.
...
2. **Objetivo (~15 min): traduzir requisito para tarefas pequenas**
   ...
   - Como fazer: decompor Favoritos/watchlist/histórico em 3 a 5 tarefas objetivas (entrada, processamento, saída, validação, evidência).
...
3. **Objetivo (~25 min): desenhar fluxo e critérios antes de implementar**
   ...
   - O que verificar: fluxo alinhado com RF16,RF17,RF18 e sem invadir BKs seguintes.
...
- **Smoke**
  - O objetivo Favoritos/watchlist/histórico pode ser demonstrado de ponta a ponta no contexto atual.
  - Não há bloqueio imediato para o BK-MF2-08.
- **Negativos**
  - Existe pelo menos 1 teste de erro/validação diretamente ligado a Favoritos/watchlist/histórico.
  - Falhas esperadas ficam explícitas e sem comportamento ambíguo.
- **Técnico**
  - Alinhamento confirmado com RF16,RF17,RF18.
...
- proof: BK BK-MF2-07 documentado com guia pedagógico completo, alinhado com backlog e requisitos associados.
