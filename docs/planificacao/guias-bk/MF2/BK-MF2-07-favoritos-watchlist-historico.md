...
- Consolidar a entrega de `Favoritos/watchlist/historico` com rastreabilidade explicita para `RF16, RF17, RF18`.
- Executar o BK `BK-MF2-07` no contexto da macro `MF2` e da sprint `S04`.
...
Entregar `Favoritos/watchlist/historico` cobrindo `RF16, RF17, RF18` na `MF2`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF2-08`).
...
- Entrega funcional de `Favoritos/watchlist/historico` com caminho principal completo.
...
- Fluxo principal de `BK-MF2-07` reproduzivel por outro colega.
...
- `docs/RF.md` e `docs/RNF.md` (itens de `RF16, RF17, RF18`).
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF2-07`).
...
3. Implementar o fluxo principal de `Favoritos/watchlist/historico`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P0`.
6. Atualizar evidence e preparar handoff para `BK-MF2-08`.

## Outputs esperados

- Output funcional de `BK-MF2-07` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF2-07
precondicoes_ok = validar_dependencias(["BK-MF2-05"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Favoritos/watchlist/historico")
...
- Condicao: fluxo principal de `BK-MF2-07` concluido ponta-a-ponta.
