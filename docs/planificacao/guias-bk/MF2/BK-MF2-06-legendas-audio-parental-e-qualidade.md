...
Neste BK vamos entregar Legendas/audio, parental e qualidade, com foco num fluxo claro e executável para alunos em contexto de PAP.
...
- Este BK fecha uma peça concreta da macro MF2 e reduz risco de bloqueios nos BKs seguintes.
- Uma execução guiada melhora consistência entre equipa, reduz retrabalho e acelera revisão técnica.
- A entrega deste BK contribui diretamente para os requisitos RF13,RF14,RF15 definidos no backlog.
- Sem este BK bem fechado, a qualidade das evidências para sprint e defesa final fica comprometida.

## O que entra (scope)

- Entregar o objetivo central: Legendas/audio, parental e qualidade.
- Produzir artefacto verificável alinhado com RF13,RF14,RF15 e com o estado atual do backlog.
...
- As dependências BK-MF2-05 foram respeitadas sem regressão funcional óbvia.
- A equipa consegue explicar o porquê da solução e não apenas o como.

## Pre-leitura mínima (10-15 min)

- docs/planificacao/backlogs/BACKLOG-MVP.md - localizar a linha do BK-MF2-06, owner, esforço e dependências.
...
- docs/RF.md e/ou docs/RNF.md - focar os códigos RF13,RF14,RF15.
...
1. **Objetivo (~10 min): confirmar ponto de partida**
   ...
   - Como fazer: validar se BK-MF2-05 está concluído ou suficientemente estável para suportar este BK.
   - O que verificar: existe baseline claro para iniciar BK-MF2-06.
...
2. **Objetivo (~15 min): traduzir o requisito em tarefas pequenas**
   ...
   - Como fazer: decompor Legendas/audio, parental e qualidade em 3 a 5 tarefas objetivas (entrada, processamento, saída, validação, evidência).
...
3. **Objetivo (~25 min): desenhar fluxo e critérios antes de implementar**
   ...
   - O que verificar: fluxo alinhado com RF13,RF14,RF15 e sem invadir BKs adjacentes.
...
- **Smoke**
  - O objetivo Legendas/audio, parental e qualidade pode ser demonstrado de ponta a ponta no contexto atual.
  - Não há bloqueio imediato para o BK-MF3-01.
- **Negativos**
  - Existe pelo menos 1 teste de erro/validação diretamente ligado a Legendas/audio, parental e qualidade.
  - Falhas esperadas ficam explícitas e sem comportamento ambíguo.
- **Técnico**
  - Alinhamento confirmado com RF13,RF14,RF15.
...
- proof: BK BK-MF2-06 documentado com guia pedagógico completo, alinhado com backlog e requisitos associados.
