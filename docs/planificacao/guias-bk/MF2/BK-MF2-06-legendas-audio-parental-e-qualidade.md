...
- Consolidar a entrega de `Legendas/audio, parental e qualidade` com rastreabilidade explicita para `RF13, RF14, RF15`.
- Executar o BK `BK-MF2-06` no contexto da macro `MF2` e da sprint `S04`.
...
Entregar `Legendas/audio, parental e qualidade` cobrindo `RF13, RF14, RF15` na `MF2`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF2-07`).
...
- Dependencias declaradas: `BK-MF2-05`.
...
- Entrega funcional de `Legendas/audio, parental e qualidade` com caminho principal completo.
...
- Fluxo principal de `BK-MF2-06` reproduzivel por outro colega.
...
- `docs/RF.md` e `docs/RNF.md` (itens de `RF13, RF14, RF15`).
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF2-06`).
...
3. Implementar o fluxo principal de `Legendas/audio, parental e qualidade`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P1`.
6. Atualizar evidence e preparar handoff para `BK-MF2-07`.

## Outputs esperados

- Output funcional de `BK-MF2-06` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF2-06
precondicoes_ok = validar_dependencias(["BK-MF2-05"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Legendas/audio, parental e qualidade")
...
- Condicao: fluxo principal de `BK-MF2-06` concluido ponta-a-ponta.
