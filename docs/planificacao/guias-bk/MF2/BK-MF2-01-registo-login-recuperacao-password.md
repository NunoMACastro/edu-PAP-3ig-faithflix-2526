...
Neste BK vamos implementar o ciclo base de identidade do utilizador: criar conta, autenticar e recuperar acesso.
...
- A execução deste BK liga diretamente os requisitos RF01,RF02,RF05 ao plano operacional do backlog.
- Sem este BK bem fechado, a validação de sprint fica frágil e a evidência para defesa perde qualidade.

## O que entra (scope)

- Entregar o objetivo central: Registo, login e recuperação de password.
- Produzir artefacto verificável alinhado com RF01,RF02,RF05 e com o estado do backlog.
...
- As dependências BK-MF1-04 foram respeitadas sem regressão funcional óbvia.
- A equipa consegue explicar o "porquê" da solução e não apenas o "como".

## Pre-leitura mínima (10-15 min)

- docs/planificacao/backlogs/BACKLOG-MVP.md - localizar a linha do BK-MF2-01, owner, esforço e dependências.
...
- docs/RF.md e/ou docs/RNF.md - focar os códigos RF01,RF02,RF05.
...
1. **Objetivo (~10 min): confirmar ponto de partida**
   ...
   - Como fazer: validar se BK-MF1-04 está concluído ou suficientemente estável para permitir progresso real.
   - O que verificar: existe um baseline claro para iniciar BK-MF2-01.
...
2. **Objetivo (~15 min): traduzir requisito para tarefas pequenas**
   ...
   - Como fazer: decompor Registo, login e recuperação de password em 3 a 5 tarefas objetivas (entrada, processamento, saída, validação, evidência).
...
3. **Objetivo (~25 min): desenhar fluxo e critérios antes de implementar**
   ...
   - O que verificar: fluxo alinhado com RF01,RF02,RF05 e sem invadir BKs seguintes.
...
- **Smoke**
  - O objetivo Registo, login e recuperação de password pode ser demonstrado de ponta a ponta no contexto atual.
  - Não há bloqueio imediato para o BK-MF2-02.
- **Negativos**
  - Existe pelo menos 1 teste de erro/validação diretamente ligado a Registo, login e recuperação de password.
  - Falhas esperadas ficam explícitas e sem comportamento ambíguo.
- **Técnico**
  - Alinhamento confirmado com RF01,RF02,RF05.
...
- proof: BK BK-MF2-01 documentado com guia pedagógico completo, alinhado com backlog e requisitos associados.
