...
- Consolidar a entrega de `Registo, login e recuperacao de password` com rastreabilidade explicita para `RF01, RF02, RF05`.
- Executar o BK `BK-MF2-01` no contexto da macro `MF2` e da sprint `S02`.
...
Entregar `Registo, login e recuperacao de password` cobrindo `RF01, RF02, RF05` na `MF2`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF2-02`).
...
- Entrega funcional de `Registo, login e recuperacao de password` com caminho principal completo.
...
- Fluxo principal de `BK-MF2-01` reproduzivel por outro colega.
...
- `docs/RF.md` e `docs/RNF.md` (itens de `RF01, RF02, RF05`).
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF2-01`).
...
- Output funcional de `BK-MF2-01` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF2-01
precondicoes_ok = validar_dependencias(["BK-MF1-04"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Registo, login e recuperacao de password")
