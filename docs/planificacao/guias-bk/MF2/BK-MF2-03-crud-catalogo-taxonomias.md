...
- Consolidar a entrega de `CRUD de catalogo e taxonomias` com rastreabilidade explicita para `RF06, RF07, RF09, RF10`.
- Executar o BK `BK-MF2-03` no contexto da macro `MF2` e da sprint `S03`.
...
Entregar `CRUD de catalogo e taxonomias` cobrindo `RF06, RF07, RF09, RF10` na `MF2`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF2-04`).
...
- Entrega funcional de `CRUD de catalogo e taxonomias` com caminho principal completo.
...
- Fluxo principal de `BK-MF2-03` reproduzivel por outro colega.
...
- `docs/RF.md` e `docs/RNF.md` (itens de `RF06, RF07, RF09, RF10`).
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF2-03`).
...
3. Implementar o fluxo principal de `CRUD de catalogo e taxonomias`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P0`.
6. Atualizar evidence e preparar handoff para `BK-MF2-04`.

## Outputs esperados

- Output funcional de `BK-MF2-03` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF2-03
precondicoes_ok = validar_dependencias(["BK-MF1-01"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("CRUD de catalogo e taxonomias")
...
- Condicao: fluxo principal de `BK-MF2-03` concluido ponta-a-ponta.
