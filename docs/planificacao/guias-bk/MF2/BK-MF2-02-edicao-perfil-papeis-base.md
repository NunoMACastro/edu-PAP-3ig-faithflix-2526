...
- Consolidar a entrega de `Edicao de perfil e papeis base` com rastreabilidade explicita para `RF03, RF04`.
- Executar o BK `BK-MF2-02` no contexto da macro `MF2` e da sprint `S03`.
...
Entregar `Edicao de perfil e papeis base` cobrindo `RF03, RF04` na `MF2`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF2-03`).
...
- Entrega funcional de `Edicao de perfil e papeis base` com caminho principal completo.
...
- Fluxo principal de `BK-MF2-02` reproduzivel por outro colega.
...
- `docs/RF.md` e `docs/RNF.md` (itens de `RF03, RF04`).
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF2-02`).
...
3. Implementar o fluxo principal de `Edicao de perfil e papeis base`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P0`.
6. Atualizar evidence e preparar handoff para `BK-MF2-03`.

## Outputs esperados

- Output funcional de `BK-MF2-02` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF2-02
precondicoes_ok = validar_dependencias(["BK-MF2-01"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Edicao de perfil e papeis base")
...
- Condicao: fluxo principal de `BK-MF2-02` concluido ponta-a-ponta.
