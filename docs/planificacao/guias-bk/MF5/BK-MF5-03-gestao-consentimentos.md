...
- Consolidar a entrega de `Gestao de consentimentos` com rastreabilidade explicita para `RF57`.
- Executar o BK `BK-MF5-03` no contexto da macro `MF5` e da sprint `S09`.
...
Entregar `Gestao de consentimentos` cobrindo `RF57` na `MF5`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF5-04`).
...
- Entrega funcional de `Gestao de consentimentos` com caminho principal completo.
...
- Fluxo principal de `BK-MF5-03` reproduzivel por outro colega.
...
- `docs/RF.md` e `docs/RNF.md` (itens de `RF57`).
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF5-03`).
...
3. Implementar o fluxo principal de `Gestao de consentimentos`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P0`.
6. Atualizar evidence e preparar handoff para `BK-MF5-04`.

## Outputs esperados

- Output funcional de `BK-MF5-03` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF5-03
precondicoes_ok = validar_dependencias(["BK-MF2-01"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Gestao de consentimentos")
...
- Condicao: fluxo principal de `BK-MF5-03` concluido ponta-a-ponta.
