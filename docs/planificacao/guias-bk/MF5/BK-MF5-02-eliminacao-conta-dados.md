...
Neste BK vamos executar eliminação de conta e dados associados com regras de segurança e consistência.
...
- A execução deste BK liga diretamente os requisitos RF56 ao plano operacional do backlog.
- Sem este BK bem fechado, a validação de sprint fica frágil e a evidência para defesa perde qualidade.

## O que entra (scope)

- Entregar o objetivo central: Eliminação de conta e dados.
- Produzir artefacto verificável alinhado com RF56 e com o estado do backlog.
...
- As dependências BK-MF5-01 foram respeitadas sem regressão funcional óbvia.
- A equipa consegue explicar o "porquê" da solução e não apenas o "como".

## Pre-leitura mínima (10-15 min)

- docs/planificacao/backlogs/BACKLOG-MVP.md - localizar a linha do BK-MF5-02, owner, esforço e dependências.
...
- docs/RF.md e/ou docs/RNF.md - focar os códigos RF56.
...
1. **Objetivo (~10 min): confirmar ponto de partida**
   ...
   - Como fazer: validar se BK-MF5-01 está concluído ou suficientemente estável para permitir progresso real.
   - O que verificar: existe um baseline claro para iniciar BK-MF5-02.
...
2. **Objetivo (~15 min): traduzir requisito para tarefas pequenas**
   ...
   - Como fazer: decompor Eliminação de conta e dados em 3 a 5 tarefas objetivas (entrada, processamento, saída, validação, evidência).
...
3. **Objetivo (~25 min): desenhar fluxo e critérios antes de implementar**
   ...
   - O que verificar: fluxo alinhado com RF56 e sem invadir BKs seguintes.
...
- **Smoke**
  - O objetivo Eliminação de conta e dados pode ser demonstrado de ponta a ponta no contexto atual.
  - Não há bloqueio imediato para o BK-MF5-03.
- **Negativos**
  - Existe pelo menos 1 teste de erro/validação diretamente ligado a Eliminação de conta e dados.
  - Falhas esperadas ficam explícitas e sem comportamento ambíguo.
- **Técnico**
  - Alinhamento confirmado com RF56.
...
- proof: BK BK-MF5-02 documentado com guia pedagógico completo, alinhado com backlog e requisitos associados.
