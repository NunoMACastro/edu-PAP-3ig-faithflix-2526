...
- Consolidar a entrega de `Exportacao de dados do utilizador` com rastreabilidade explicita para `RF55`.
- Executar o BK `BK-MF5-01` no contexto da macro `MF5` e da sprint `S09`.
...
Entregar `Exportacao de dados do utilizador` cobrindo `RF55` na `MF5`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF5-02`).
...
- Entrega funcional de `Exportacao de dados do utilizador` com caminho principal completo.
...
- Fluxo principal de `BK-MF5-01` reproduzivel por outro colega.
...
- `docs/RF.md` e `docs/RNF.md` (itens de `RF55`).
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF5-01`).
...
3. Implementar o fluxo principal de `Exportacao de dados do utilizador`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P0`.
6. Atualizar evidence e preparar handoff para `BK-MF5-02`.

## Outputs esperados

- Output funcional de `BK-MF5-01` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF5-01
precondicoes_ok = validar_dependencias(["BK-MF2-01"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Exportacao de dados do utilizador")
...
- Condicao: fluxo principal de `BK-MF5-01` concluido ponta-a-ponta.
