...
- `estado`: `TODO`
- `esforco`: `L`
- `dependencias`: `BK-MF2-02,BK-MF2-05`
- `rf_rnf`: `RF61, RF62`
- `fase_documental`: `Fase 2`
- `sprint`: `S10`
- `core_or_reforco`: `Core`
- `proximo_bk`: `BK-MF5-08`
- `guia_path`: `docs/planificacao/guias-bk/MF5/BK-MF5-07-perfis-familiares-e-dispositivos.md`
...
- Consolidar a entrega de `Perfis familiares e dispositivos` com rastreabilidade explicita para `RF61, RF62`.
- Executar o BK `BK-MF5-07` no contexto da macro `MF5` e da sprint `S10`.
...
Entregar `Perfis familiares e dispositivos` cobrindo `RF61, RF62` na `MF5`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF5-08`).
...
- Dependencias declaradas: `BK-MF2-02,BK-MF2-05`.
...
- Entrega funcional de `Perfis familiares e dispositivos` com caminho principal completo.
...
- Fluxo principal de `BK-MF5-07` reproduzivel por outro colega.
...
- `docs/RF.md` e `docs/RNF.md` (itens de `RF61, RF62`).
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF5-07`).
...
3. Implementar o fluxo principal de `Perfis familiares e dispositivos`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P1`.
6. Atualizar evidence e preparar handoff para `BK-MF5-08`.

## Outputs esperados

- Output funcional de `BK-MF5-07` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF5-07
precondicoes_ok = validar_dependencias(["BK-MF2-02","BK-MF2-05"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Perfis familiares e dispositivos")
...
- Condicao: fluxo principal de `BK-MF5-07` concluido ponta-a-ponta.
