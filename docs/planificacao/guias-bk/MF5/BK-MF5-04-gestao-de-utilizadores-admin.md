...
Definir que roles e estados podem ser alterados por um administrador.
...
Mantém `VALID_ROLES` e acrescenta estados operacionais.
...
O validator aceita alteração parcial: só role, só estado ou ambos. Valores fora da lista fechada falham com `400`. Isto evita roles inventadas e estados impossíveis.
...
```bash
cd real_dev/backend
node -e "import('./src/modules/users/user.validation.js').then(({ assertAdminUserUpdate }) => console.log(assertAdminUserUpdate({ role: 'moderator', accountStatus: 'active' }).role))"
```

O resultado esperado é `moderator`.
...
import { assertAdminUserUpdate } from "./user.validation.js";
...
`buildAdminUserQuery` permite filtros simples sem expor campos internos. `updateUserByAdmin` protege contra auto-despromoção e auto-bloqueio. A ação fica registada em `admin_audit_logs`, cumprindo o princípio de auditoria para operações críticas. `updateUserRole` só fica no ficheiro para compatibilidade com a rota antiga; o contrato novo deste BK é `PATCH /api/users/:id/admin`.
...
Provar que roles e estados fora do contrato são rejeitados.
...
import { assertAdminUserUpdate } from "../../src/modules/users/user.validation.js";
...
O teste cobre caso válido, role inválida, estado inválido e pedido vazio. Isto protege a fronteira do endpoint.
