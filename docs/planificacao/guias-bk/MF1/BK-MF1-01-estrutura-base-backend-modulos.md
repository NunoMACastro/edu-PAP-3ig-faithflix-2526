...
- Consolidar a entrega de `Estrutura base backend por modulos` com rastreabilidade explicita para `RNF27`.
- Executar o BK `BK-MF1-01` no contexto da macro `MF1` e da sprint `S01`.
...
Entregar `Estrutura base backend por modulos` cobrindo `RNF27` na `MF1`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF1-02`).
...
- Entrega funcional de `Estrutura base backend por modulos` com caminho principal completo.
...
- Fluxo principal de `BK-MF1-01` reproduzivel por outro colega.
...
- `docs/RF.md` e `docs/RNF.md` (itens de `RNF27`).
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF1-01`).
...
3. Implementar o fluxo principal de `Estrutura base backend por modulos`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P0`.
6. Atualizar evidence e preparar handoff para `BK-MF1-02`.

## Outputs esperados

- Output funcional de `BK-MF1-01` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF1-01
precondicoes_ok = validar_dependencias(["BK-MF0-06"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Estrutura base backend por modulos")
...
- Condicao: fluxo principal de `BK-MF1-01` concluido ponta-a-ponta.
