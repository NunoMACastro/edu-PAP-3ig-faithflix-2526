...
Neste BK vamos montar a estrutura modular inicial do backend para separar domínios e reduzir acoplamento.
...
- A execução deste BK liga diretamente os requisitos RNF27 ao plano operacional do backlog.
- Sem este BK bem fechado, a validação de sprint fica frágil e a evidência para defesa perde qualidade.

## O que entra (scope)

- Entregar o objetivo central: Estrutura base backend por módulos.
- Produzir artefacto verificável alinhado com RNF27 e com o estado do backlog.
...
- docs/planificacao/backlogs/BACKLOG-MVP.md - localizar a linha do BK-MF1-01, owner, esforço e dependências.
...
- docs/RF.md e/ou docs/RNF.md - focar os códigos RNF27.
...
1. **Objetivo (~10 min): confirmar ponto de partida**
   ...
   - O que verificar: existe um baseline claro para iniciar BK-MF1-01.
...
2. **Objetivo (~15 min): traduzir requisito para tarefas pequenas**
   ...
   - Como fazer: decompor Estrutura base backend por módulos em 3 a 5 tarefas objetivas (entrada, processamento, saída, validação, evidência).
...
3. **Objetivo (~25 min): desenhar fluxo e critérios antes de implementar**
   ...
   - O que verificar: fluxo alinhado com RNF27 e sem invadir BKs seguintes.
...
- **Smoke**
  - O objetivo Estrutura base backend por módulos pode ser demonstrado de ponta a ponta no contexto atual.
  - Não há bloqueio imediato para o BK-MF1-02.
- **Negativos**
  - Existe pelo menos 1 teste de erro/validação diretamente ligado a Estrutura base backend por módulos.
  - Falhas esperadas ficam explícitas e sem comportamento ambíguo.
- **Técnico**
  - Alinhamento confirmado com RNF27.
...
- proof: BK BK-MF1-01 documentado com guia pedagógico completo, alinhado com backlog e requisitos associados.
