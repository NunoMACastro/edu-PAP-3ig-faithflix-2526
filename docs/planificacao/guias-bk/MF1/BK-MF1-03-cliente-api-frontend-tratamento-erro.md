...
Entregar `Cliente API frontend com tratamento de erro` cobrindo `RNF05, RNF30` na `MF1`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF1-04`).
...
- Entrega funcional de `Cliente API frontend com tratamento de erro` com caminho principal completo.
...
- Fluxo principal de `BK-MF1-03` reproduzivel por outro colega.
...
3. Implementar o fluxo principal de `Cliente API frontend com tratamento de erro`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P0`.
6. Atualizar evidence e preparar handoff para `BK-MF1-04`.

## Outputs esperados

- Output funcional de `BK-MF1-03` concluido sem blocker.
...
precondicoes_ok = validar_dependencias(["BK-MF1-02"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Cliente API frontend com tratamento de erro")
...
- Condicao: fluxo principal de `BK-MF1-03` concluido ponta-a-ponta.
