...
Neste BK vamos criar camada de comunicação com API no frontend com tratamento de erro claro e rastreável.
...
- A execução deste BK liga diretamente os requisitos RNF05,RNF30 ao plano operacional do backlog.
- Sem este BK bem fechado, a validação de sprint fica frágil e a evidência para defesa perde qualidade.

## O que entra (scope)

- Entregar o objetivo central: Cliente API frontend com tratamento de erro.
- Produzir artefacto verificável alinhado com RNF05,RNF30 e com o estado do backlog.
...
- As dependências BK-MF1-02 foram respeitadas sem regressão funcional óbvia.
- A equipa consegue explicar o "porquê" da solução e não apenas o "como".

## Pre-leitura mínima (10-15 min)

- docs/planificacao/backlogs/BACKLOG-MVP.md - localizar a linha do BK-MF1-03, owner, esforço e dependências.
...
- docs/RF.md e/ou docs/RNF.md - focar os códigos RNF05,RNF30.
...
1. **Objetivo (~10 min): confirmar ponto de partida**
   ...
   - Como fazer: validar se BK-MF1-02 está concluído ou suficientemente estável para permitir progresso real.
   - O que verificar: existe um baseline claro para iniciar BK-MF1-03.
...
2. **Objetivo (~15 min): traduzir requisito para tarefas pequenas**
   ...
   - Como fazer: decompor Cliente API frontend com tratamento de erro em 3 a 5 tarefas objetivas (entrada, processamento, saída, validação, evidência).
...
3. **Objetivo (~25 min): desenhar fluxo e critérios antes de implementar**
   ...
   - O que verificar: fluxo alinhado com RNF05,RNF30 e sem invadir BKs seguintes.
...
- **Smoke**
  - O objetivo Cliente API frontend com tratamento de erro pode ser demonstrado de ponta a ponta no contexto atual.
  - Não há bloqueio imediato para o BK-MF1-04.
- **Negativos**
  - Existe pelo menos 1 teste de erro/validação diretamente ligado a Cliente API frontend com tratamento de erro.
  - Falhas esperadas ficam explícitas e sem comportamento ambíguo.
- **Técnico**
  - Alinhamento confirmado com RNF05,RNF30.
...
- proof: BK BK-MF1-03 documentado com guia pedagógico completo, alinhado com backlog e requisitos associados.
