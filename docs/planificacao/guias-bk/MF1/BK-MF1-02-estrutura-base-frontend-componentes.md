...
Entregar `Estrutura base frontend por componentes` cobrindo `RNF28` na `MF1`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF1-03`).
...
- Entrega funcional de `Estrutura base frontend por componentes` com caminho principal completo.
...
- Fluxo principal de `BK-MF1-02` reproduzivel por outro colega.
...
3. Implementar o fluxo principal de `Estrutura base frontend por componentes`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P0`.
6. Atualizar evidence e preparar handoff para `BK-MF1-03`.

## Outputs esperados

- Output funcional de `BK-MF1-02` concluido sem blocker.
...
resultado = executar_fluxo_principal("Estrutura base frontend por componentes")
...
- Condicao: fluxo principal de `BK-MF1-02` concluido ponta-a-ponta.
