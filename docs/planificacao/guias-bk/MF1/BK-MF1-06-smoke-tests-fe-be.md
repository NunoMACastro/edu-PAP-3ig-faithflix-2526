...
- Consolidar a entrega de `Smoke tests FE/BE` com rastreabilidade explicita para `RNF29`.
- Executar o BK `BK-MF1-06` no contexto da macro `MF1` e da sprint `S02`.
...
Entregar `Smoke tests FE/BE` cobrindo `RNF29` na `MF1`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF2-01`).
...
- Dependencias declaradas: `BK-MF1-03,BK-MF1-04`.
...
- Entrega funcional de `Smoke tests FE/BE` com caminho principal completo.
...
- Fluxo principal de `BK-MF1-06` reproduzivel por outro colega.
...
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF1-06`).
...
3. Implementar o fluxo principal de `Smoke tests FE/BE`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P1`.
6. Atualizar evidence e preparar handoff para `BK-MF2-01`.

## Outputs esperados

- Output funcional de `BK-MF1-06` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF1-06
precondicoes_ok = validar_dependencias(["BK-MF1-03","BK-MF1-04"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Smoke tests FE/BE")
...
- Condicao: fluxo principal de `BK-MF1-06` concluido ponta-a-ponta.
