...
Neste BK vamos entregar Smoke tests FE/BE, com foco num fluxo claro e executável para alunos em contexto de PAP.
...
- A entrega deste BK contribui diretamente para os requisitos RNF29 definidos no backlog.
- Sem este BK bem fechado, a qualidade das evidências para sprint e defesa final fica comprometida.

## O que entra (scope)

- Entregar o objetivo central: Smoke tests FE/BE.
- Produzir artefacto verificável alinhado com RNF29 e com o estado atual do backlog.
...
- As dependências BK-MF1-03,BK-MF1-04 foram respeitadas sem regressão funcional óbvia.
- A equipa consegue explicar o porquê da solução e não apenas o como.

## Pre-leitura mínima (10-15 min)

- docs/planificacao/backlogs/BACKLOG-MVP.md - localizar a linha do BK-MF1-06, owner, esforço e dependências.
...
- docs/RF.md e/ou docs/RNF.md - focar os códigos RNF29.
...
1. **Objetivo (~10 min): confirmar ponto de partida**
   ...
   - Como fazer: validar se BK-MF1-03,BK-MF1-04 está concluído ou suficientemente estável para suportar este BK.
   - O que verificar: existe baseline claro para iniciar BK-MF1-06.
...
2. **Objetivo (~15 min): traduzir o requisito em tarefas pequenas**
   ...
   - Como fazer: decompor Smoke tests FE/BE em 3 a 5 tarefas objetivas (entrada, processamento, saída, validação, evidência).
...
3. **Objetivo (~25 min): desenhar fluxo e critérios antes de implementar**
   ...
   - O que verificar: fluxo alinhado com RNF29 e sem invadir BKs adjacentes.
...
- **Smoke**
  - O objetivo Smoke tests FE/BE pode ser demonstrado de ponta a ponta no contexto atual.
  - Não há bloqueio imediato para o BK-MF2-06.
- **Negativos**
  - Existe pelo menos 1 teste de erro/validação diretamente ligado a Smoke tests FE/BE.
  - Falhas esperadas ficam explícitas e sem comportamento ambíguo.
- **Técnico**
  - Alinhamento confirmado com RNF29.
...
- proof: BK BK-MF1-06 documentado com guia pedagógico completo, alinhado com backlog e requisitos associados.
