# BK-MF1-04 - Sessão segura backend (cookies e auth base)
...
Neste BK vamos estabelecer autenticação e sessão segura base no backend com cookies e controlo de acesso inicial.
...
- A execução deste BK liga diretamente os requisitos RNF13,RNF15 ao plano operacional do backlog.
- Sem este BK bem fechado, a validação de sprint fica frágil e a evidência para defesa perde qualidade.

## O que entra (scope)

- Entregar o objetivo central: Sessão segura backend (cookies e auth base).
- Produzir artefacto verificável alinhado com RNF13,RNF15 e com o estado do backlog.
...
- docs/planificacao/backlogs/BACKLOG-MVP.md - localizar a linha do BK-MF1-04, owner, esforço e dependências.
...
- docs/RF.md e/ou docs/RNF.md - focar os códigos RNF13,RNF15.
...
1. **Objetivo (~10 min): confirmar ponto de partida**
   ...
   - Como fazer: validar se BK-MF1-01 está concluído ou suficientemente estável para permitir progresso real.
   - O que verificar: existe um baseline claro para iniciar BK-MF1-04.
...
2. **Objetivo (~15 min): traduzir requisito para tarefas pequenas**
   ...
   - Como fazer: decompor Sessão segura backend (cookies e auth base) em 3 a 5 tarefas objetivas (entrada, processamento, saída, validação, evidência).
...
3. **Objetivo (~25 min): desenhar fluxo e critérios antes de implementar**
   ...
   - O que verificar: fluxo alinhado com RNF13,RNF15 e sem invadir BKs seguintes.
...
- **Smoke**
  - O objetivo Sessão segura backend (cookies e auth base) pode ser demonstrado de ponta a ponta no contexto atual.
  - Não há bloqueio imediato para o BK-MF2-01.
- **Negativos**
  - Existe pelo menos 1 teste de erro/validação diretamente ligado a Sessão segura backend (cookies e auth base).
  - Falhas esperadas ficam explícitas e sem comportamento ambíguo.
- **Técnico**
  - Alinhamento confirmado com RNF13,RNF15.
...
- proof: BK BK-MF1-04 documentado com guia pedagógico completo, alinhado com backlog e requisitos associados.
