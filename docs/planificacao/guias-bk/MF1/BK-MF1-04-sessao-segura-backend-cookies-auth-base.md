...
- Consolidar a entrega de `Sessao segura backend (cookies e auth base)` com rastreabilidade explicita para `RNF13, RNF15`.
- Executar o BK `BK-MF1-04` no contexto da macro `MF1` e da sprint `S02`.
...
Entregar `Sessao segura backend (cookies e auth base)` cobrindo `RNF13, RNF15` na `MF1`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF1-05`).
...
- Entrega funcional de `Sessao segura backend (cookies e auth base)` com caminho principal completo.
...
- Fluxo principal de `BK-MF1-04` reproduzivel por outro colega.
...
- `docs/RF.md` e `docs/RNF.md` (itens de `RNF13, RNF15`).
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF1-04`).
...
3. Implementar o fluxo principal de `Sessao segura backend (cookies e auth base)`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P0`.
6. Atualizar evidence e preparar handoff para `BK-MF1-05`.

## Outputs esperados

- Output funcional de `BK-MF1-04` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF1-04
precondicoes_ok = validar_dependencias(["BK-MF1-01"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Sessao segura backend (cookies e auth base)")
...
- Condicao: fluxo principal de `BK-MF1-04` concluido ponta-a-ponta.
