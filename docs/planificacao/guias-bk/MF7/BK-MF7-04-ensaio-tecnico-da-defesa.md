# BK-MF7-04 - Ensaio tecnico da defesa
...
- Consolidar a entrega de `Ensaio tecnico da defesa` com rastreabilidade explicita para `transversal`.
- Executar o BK `BK-MF7-04` no contexto da macro `MF7` e da sprint `S12`.
...
Entregar `Ensaio tecnico da defesa` cobrindo `transversal` na `MF7`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF7-05`).
...
- Dependencias declaradas: `BK-MF7-03`.
...
- Entrega funcional de `Ensaio tecnico da defesa` com caminho principal completo.
...
- Fluxo principal de `BK-MF7-04` reproduzivel por outro colega.
...
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF7-04`).
...
3. Implementar o fluxo principal de `Ensaio tecnico da defesa`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P1`.
6. Atualizar evidence e preparar handoff para `BK-MF7-05`.

## Outputs esperados

- Output funcional de `BK-MF7-04` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF7-04
precondicoes_ok = validar_dependencias(["BK-MF7-03"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Ensaio tecnico da defesa")
...
- Condicao: fluxo principal de `BK-MF7-04` concluido ponta-a-ponta.
