...
Neste BK vamos entregar Matriz de cobertura RNF -> validação, com foco num fluxo claro e executável para alunos em contexto de PAP.
...
- A entrega deste BK contribui diretamente para os requisitos RNF01..RNF40 definidos no backlog.
- Sem este BK bem fechado, a qualidade das evidências para sprint e defesa final fica comprometida.

## O que entra (scope)

- Entregar o objetivo central: Matriz de cobertura RNF -> validação.
- Produzir artefacto verificável alinhado com RNF01..RNF40 e com o estado atual do backlog.
...
- docs/planificacao/backlogs/BACKLOG-MVP.md - localizar a linha do BK-MF7-02, owner, esforço e dependências.
...
- docs/RF.md e/ou docs/RNF.md - focar os códigos RNF01..RNF40.
...
1. **Objetivo (~10 min): confirmar ponto de partida**
   ...
   - O que verificar: existe baseline claro para iniciar BK-MF7-02.
...
2. **Objetivo (~15 min): traduzir o requisito em tarefas pequenas**
   ...
   - Como fazer: decompor Matriz de cobertura RNF -> validação em 3 a 5 tarefas objetivas (entrada, processamento, saída, validação, evidência).
...
3. **Objetivo (~25 min): desenhar fluxo e critérios antes de implementar**
   ...
   - O que verificar: fluxo alinhado com RNF01..RNF40 e sem invadir BKs adjacentes.
...
- **Smoke**
  - O objetivo Matriz de cobertura RNF -> validação pode ser demonstrado de ponta a ponta no contexto atual.
  - Não há bloqueio imediato para o BK-MF7-03.
- **Negativos**
  - Existe pelo menos 1 teste de erro/validação diretamente ligado a Matriz de cobertura RNF -> validação.
  - Falhas esperadas ficam explícitas e sem comportamento ambíguo.
- **Técnico**
  - Alinhamento confirmado com RNF01..RNF40.
...
- proof: BK BK-MF7-02 documentado com guia pedagógico completo, alinhado com backlog e requisitos associados.
