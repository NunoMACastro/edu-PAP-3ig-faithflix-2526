...
Neste BK vamos entregar Notificações transacionais e preferências, com foco num fluxo claro e executável para alunos em contexto de PAP.
...
- A entrega deste BK contribui diretamente para os requisitos RF52,RF53,RF54 definidos no backlog.
- Sem este BK bem fechado, a qualidade das evidências para sprint e defesa final fica comprometida.

## O que entra (scope)

- Entregar o objetivo central: Notificações transacionais e preferências.
- Produzir artefacto verificável alinhado com RF52,RF53,RF54 e com o estado atual do backlog.
...
- As dependências BK-MF4-01 foram respeitadas sem regressão funcional óbvia.
- A equipa consegue explicar o porquê da solução e não apenas o como.

## Pre-leitura mínima (10-15 min)

- docs/planificacao/backlogs/BACKLOG-MVP.md - localizar a linha do BK-MF4-08, owner, esforço e dependências.
...
- docs/RF.md e/ou docs/RNF.md - focar os códigos RF52,RF53,RF54.
...
1. **Objetivo (~10 min): confirmar ponto de partida**
   ...
   - Como fazer: validar se BK-MF4-01 está concluído ou suficientemente estável para suportar este BK.
   - O que verificar: existe baseline claro para iniciar BK-MF4-08.
...
2. **Objetivo (~15 min): traduzir o requisito em tarefas pequenas**
   ...
   - Como fazer: decompor Notificações transacionais e preferências em 3 a 5 tarefas objetivas (entrada, processamento, saída, validação, evidência).
...
3. **Objetivo (~25 min): desenhar fluxo e critérios antes de implementar**
   ...
   - O que verificar: fluxo alinhado com RF52,RF53,RF54 e sem invadir BKs adjacentes.
...
- **Smoke**
  - O objetivo Notificações transacionais e preferências pode ser demonstrado de ponta a ponta no contexto atual.
  - Não há bloqueio imediato para o BK-MF5-04.
- **Negativos**
  - Existe pelo menos 1 teste de erro/validação diretamente ligado a Notificações transacionais e preferências.
  - Falhas esperadas ficam explícitas e sem comportamento ambíguo.
- **Técnico**
  - Alinhamento confirmado com RF52,RF53,RF54.
...
- proof: BK BK-MF4-08 documentado com guia pedagógico completo, alinhado com backlog e requisitos associados.
