...
- Consolidar a entrega de `Notificacoes transacionais e preferencias` com rastreabilidade explicita para `RF52, RF53, RF54`.
- Executar o BK `BK-MF4-08` no contexto da macro `MF4` e da sprint `S07`.
...
Entregar `Notificacoes transacionais e preferencias` cobrindo `RF52, RF53, RF54` na `MF4`, com fluxo principal verificavel e evidencia tecnica pronta para gate.
...
- Entrega funcional de `Notificacoes transacionais e preferencias` com caminho principal completo.
...
- Fluxo principal de `BK-MF4-08` reproduzivel por outro colega.
...
- `docs/RF.md` e `docs/RNF.md` (itens de `RF52, RF53, RF54`).
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF4-08`).
...
3. Implementar o fluxo principal de `Notificacoes transacionais e preferencias`.
...
- Output funcional de `BK-MF4-08` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF4-08
precondicoes_ok = validar_dependencias(["BK-MF4-01"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Notificacoes transacionais e preferencias")
...
- Condicao: fluxo principal de `BK-MF4-08` concluido ponta-a-ponta.
