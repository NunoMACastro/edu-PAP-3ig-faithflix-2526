# BK-MF4-07 - Workflow editorial e denuncias
...
- `guia_path`: `docs/planificacao/guias-bk/MF4/BK-MF4-07-workflow-editorial-e-denuncias.md`
...
- Consolidar a entrega de `Workflow editorial e denuncias` com rastreabilidade explicita para `RF49, RF50, RF51`.
- Executar o BK `BK-MF4-07` no contexto da macro `MF4` e da sprint `S08`.
...
Entregar `Workflow editorial e denuncias` cobrindo `RF49, RF50, RF51` na `MF4`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF4-08`).
...
- Dependencias declaradas: `BK-MF3-02`.
...
- Entrega funcional de `Workflow editorial e denuncias` com caminho principal completo.
...
- Fluxo principal de `BK-MF4-07` reproduzivel por outro colega.
...
- `docs/RF.md` e `docs/RNF.md` (itens de `RF49, RF50, RF51`).
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF4-07`).
...
3. Implementar o fluxo principal de `Workflow editorial e denuncias`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P1`.
6. Atualizar evidence e preparar handoff para `BK-MF4-08`.

## Outputs esperados

- Output funcional de `BK-MF4-07` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF4-07
precondicoes_ok = validar_dependencias(["BK-MF3-02"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Workflow editorial e denuncias")
...
- Condicao: fluxo principal de `BK-MF4-07` concluido ponta-a-ponta.
