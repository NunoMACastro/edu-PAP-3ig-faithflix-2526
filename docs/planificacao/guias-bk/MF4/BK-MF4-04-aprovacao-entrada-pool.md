...
- Consolidar a entrega de `Aprovacao e entrada na pool` com rastreabilidade explicita para `RF42, RF43`.
- Executar o BK `BK-MF4-04` no contexto da macro `MF4` e da sprint `S08`.
...
Entregar `Aprovacao e entrada na pool` cobrindo `RF42, RF43` na `MF4`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF4-05`).
...
- Dependencias declaradas: `BK-MF4-03`.
...
- Entrega funcional de `Aprovacao e entrada na pool` com caminho principal completo.
...
- Fluxo principal de `BK-MF4-04` reproduzivel por outro colega.
...
- `docs/RF.md` e `docs/RNF.md` (itens de `RF42, RF43`).
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF4-04`).
...
3. Implementar o fluxo principal de `Aprovacao e entrada na pool`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P0`.
6. Atualizar evidence e preparar handoff para `BK-MF4-05`.

## Outputs esperados

- Output funcional de `BK-MF4-04` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF4-04
precondicoes_ok = validar_dependencias(["BK-MF4-03"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Aprovacao e entrada na pool")
...
- Condicao: fluxo principal de `BK-MF4-04` concluido ponta-a-ponta.
