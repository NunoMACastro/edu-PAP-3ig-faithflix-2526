...
- Consolidar a entrega de `Candidaturas de associacoes` com rastreabilidade explicita para `RF41`.
- Executar o BK `BK-MF4-03` no contexto da macro `MF4` e da sprint `S08`.
...
Entregar `Candidaturas de associacoes` cobrindo `RF41` na `MF4`, com fluxo principal verificavel e evidencia tecnica pronta para gate.

## Porque isto e importante

- Fecha capacidade critica desta macro sem criar drift de backlog.
- Reduz risco tecnico para o proximo BK da sequencia (`BK-MF4-04`).
...
- Dependencias declaradas: `BK-MF1-04`.
...
- Entrega funcional de `Candidaturas de associacoes` com caminho principal completo.
...
- Fluxo principal de `BK-MF4-03` reproduzivel por outro colega.
...
- `docs/RF.md` e `docs/RNF.md` (itens de `RF41`).
- `docs/planificacao/backlogs/BACKLOG-MVP.md` (linha de `BK-MF4-03`).
...
3. Implementar o fluxo principal de `Candidaturas de associacoes`.
4. Executar smoke e validar integracao com BKs adjacentes.
5. Executar negativos obrigatorios para `P0`.
6. Atualizar evidence e preparar handoff para `BK-MF4-04`.

## Outputs esperados

- Output funcional de `BK-MF4-03` concluido sem blocker.
...
```text
# pseudo-checklist BK-MF4-03
precondicoes_ok = validar_dependencias(["BK-MF1-04"])
assert precondicoes_ok == true

resultado = executar_fluxo_principal("Candidaturas de associacoes")
...
- Condicao: fluxo principal de `BK-MF4-03` concluido ponta-a-ponta.
